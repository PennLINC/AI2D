...
The exact **fMRIPrep** pipeline is documented in the *boilerplate* located at `<fMRIPrep_output>/logs/CITATION.html`. If the data were processed with [BABS]({{ site.baseurl }}/docs/imaging/image_babs/), refer to [Get Data]({{ site.baseurl }}/docs/get_data#312-explore-the-data-structure) for details on unzipping `<fMRIPrep_output>` to access the boilerplate file.
