...
<br>
Functional MRI data were preprocessed using **[fMRIPrep](https://fmriprep.org/en/stable/)**, which included motion correction, slice-timing correction (if `SliceTime` metadata is available), aligning the estimated fieldmap (if available) to func with rigid-registration, and registration to T1w. Confound timeseries included FD, DVARS, and global CSF/WM/whole-brain signals. Physiological regressors were derived with CompCor, retaining enough components to explain 50% of variance, with tCompCor from the top 2% most variable voxels and aCompCor from CSF and WM masks. Data were registered to MNI152NLin6Asym, as well as resampled into fsLR space with 91k grayordinates, and all transformations composed into a single cubic B-spline interpolation step.
